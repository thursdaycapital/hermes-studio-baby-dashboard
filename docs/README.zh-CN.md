...
更简单的方法是点击“从 GitHub 在线更新”。设备会通过 HTTPS 直接下载本仓库由
GitHub Pages 分发的最新版 `quote0_baby.bin`，验证固件后写入备用 OTA 分区并自动重启。
使用 Pages CDN 是为了避开 Release 附件的跨域限制和多次重定向；固件来源仍是本仓库。
手机只负责启动和显示进度，设备本身必须能通过当前 Wi-Fi 访问互联网。
